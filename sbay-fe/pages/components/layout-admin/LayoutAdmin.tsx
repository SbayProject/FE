        ...
        <div className="min-h-screen flex flex-col">
            <header className="bg-red-500 text-white py-4 flex items-center justify-between">
                ...
                <div className="flex items-center mr-5">
                    ...
                    />
                    <span className="mr-5">Tiến Đạt</span>
                </div>
            </header>

            <div className="flex-grow container mx-auto py-8 flex">
                {isMobile && (
                    <nav className={`dark:text-black fixed top-0 right-0 h-full w-64 bg-white z-50 transition-transform transform ${menuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
                        <div className="md:hidden">
                            <button onClick={toggleMenu} className="text-red-500 hover:text-gray-400 p-2 md:justify-end">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" className={`w-6 h-6 ${menuOpen ? 'transform rotate-180' : ''}`}>
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 6h16M4 12h16M4 18h16" />
                                </svg>
                            </button>
                            <div className={`${menuOpen ? 'block' : 'hidden'} w-[250px] bg-white w-full h-full transition-transform transform`}>
                                <ul className="flex flex-col items-end">
                                    <li className="mb-2">
                                        <Link href="/components/admin-management/manage-editor" className="hover:text-gray-400 py-2 px-4 block" onClick={closeMenu}>
                                            Quản lý nhân viên
                                        </Link>
                                    </li>
                                    <li className="mb-2">
                                        <Link href="/components/admin-management/manage-post" className="hover:text-gray-400 py-2 px-4 block" onClick={closeMenu}>
                                            Quản lý bài viết
                                        </Link>
                                    </li>
                                    <li className="mb-2">
                                        <Link href="#" className="hover:text-gray-400 py-2 px-4 block" onClick={closeMenu}>
